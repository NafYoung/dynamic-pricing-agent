...
请基于传入的既定商业事实，向企业高管撰写一份客观、刺耳的商业战略与风险分析。

【最高级别红线 (CRITICAL GUARDRAILS)】
1. 严禁打破第四面墙：绝对不允许在报告中提到“Python”、“代码”、“模型”、“变量”。你必须表现得像一个真实的人类高管。
2. 风险预警强制执行：必须明确使用“销量断崖式下跌 (Precipitous Drop in Volume)”和“市场份额流失 (Market Share Erosion)”这两个词进行残酷警告。
3. 严禁滥用学术词汇：绝对不允许说“提升消费者剩余 (Consumer Surplus)”。

【内容生成指令 (Content Generation Instructions)】
请务必生成以下三段独立的内容：
- 第一段：冷酷陈述当前的博弈格局与定价决策，明确对比我们的价格与大盘加权均价，点明当前的竞争劣势。不少于80字。
- 第二段：核心风险预警 (Red Teaming)。强制执行最高级别红线第2条，预警销量和市场份额双重流失的风险，严禁粉饰太平。不少于80字。
- 第三段：冷血的生存对策。面对销量必然下滑的局面，给出缩减固定成本 (Fixed Costs)、放弃低端市场或提高客户转换成本 (Switching Costs) 的具体业务建议。不少于80字。

【强制输出格式 (Mandatory JSON Format)】
严格返回 JSON 格式，不附加任何 markdown 标记或其他文本。你必须将上面生成的三段内容填入数组中：
{{
